...
//Os usuários poderiam estar em um Set, jã que não podem se repetir na lista
        List<Usuario> usuarios = Arrays.asList(usuario1, usuario2, usuario3, usuario4, usuario5, usuario6);

        List<Cliente> clientes = Arrays.asList(cliente, cliente2);
        //As empresas também poderiam estar em um Set, jã que não podem se repetir na lista
        List<Empresa> empresas = Arrays.asList(empresa, empresa2, empresa3);
        //Os produtos poderiam estar em um Map, para serem separados por chave valor
    ...
    //Método muito extenso, com varias funções que poderiam estar separadas em métodos e classe mais genéricas.
    public static void executar(List<Usuario> usuarios, List<Cliente> clientes, List<Empresa> empresas,
                                List<Produto> produtos, List<Produto> carrinho, List<Venda> vendas) {//Método sem tratamento de exceções, a resposta para o cliente em caso de erro não será adequada
        Scanner sc = new Scanner(System.in);//Não chamou o método 'Close()' da classe Scanner, o teclado ficará aberto
        //mesmo após o teemino do uso.
        // Uma boa opcão seria colocar a classe Scanner em um 'try with resources', para assim que o try acabar,
        // fechar também o teclado, pois a classe Scanner é uma implmentação da classe 'Closeable'.

        System.out.println("Entre com seu usuário e senha:");
        System.out.print("Usuário: ");
        String username = sc.next();
        System.out.print("Senha: ");
        String senha = sc.next();//Pode haver exceção com a entrada de dados do tipo errado.


        List<Usuario> usuariosSearch = usuarios.stream().filter(x -> x.getUsername().equals(username))
                .collect(Collectors.toList());
        //Muitas estruturas de decisão, código precia de refatoração
        if (usuariosSearch.size() > 0) {
            Usuario usuarioLogado = usuariosSearch.get(0);
            if ((usuarioLogado.getSenha().equals(senha))) {

                System.out.println("Escolha uma opção para iniciar");
                if (usuarioLogado.IsEmpresa()) {//Poderiam ser 2 metodos distintos, assim evitaria o uso do switch e diminuiria o alto acoplamento.
                    ...
                    switch (escolha) {
                        ...
                        }// Falta do case 'Default', para caso o usuário retorne um valor inesperado.
                    }

                } else {//Tudo daqui para baixo poderia estar separado em um método ou classe separada
                    System.out.println("1 - Relizar Compras");
                    System.out.println("2 - Ver Compras");//Poderiam ser 2 metodos distintos, assim evitaria o uso do switch e diminuiria o alto acoplamento.
                    System.out.println("0 - Deslogar");
                    Integer escolha = sc.nextInt();
                    switch (escolha) {
                        case 1: {
    ...
    //Método sem tratamento de exceção, a resposta para o usuário não será adequada.
